...
-------------------------------------------------------------
 ...
 5%            0              0
10%            0              0       Obs               2,994
25%            0              0       Sum of wgt.       2,994

50%            0                      Mean           .2334669
                        Largest       Std. dev.      .4231075
75%            0              1
90%            1              1       Variance       .1790199
95%            1              1       Skewness       1.260094
99%            1              1       Kurtosis       2.587837
*/
...
50%            0                      Mean           .4128257
                        Largest       Std. dev.      .4924242
75%            1              1
90%            1              1       Variance       .2424816
95%            1              1       Skewness       .3541211
99%            1              1       Kurtosis       1.125402
*/
...
eststo: estpost sum ///
	...
	, detail
esttab using "$DESCRIPT/250922sumstats.tex", replace label ///
...
eststo: qui reg frac_rev city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250917_issuerlvl_debtchoice_allgo.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and debt choice (All GO Vote)") star(* .10 ** .05 *** .01) ///
	s(N r2_a SE, fmt(%9.0fc 3) label ("N" "Adj. $ R^2$" "Cluster")) ///
	/*drop(ln_gdp ln_pers_inc ln_percap_inc ln_emp _cons)*/ label booktabs noobs nonotes nogaps ///
	order(city_go_vote $countydemo glm_proactive state_ltgo_allowed state_go_vote)
	
*Run version of this where Louisiana is LTGO Allowed = 0
/*
replace state_ltgo_allowed = 0 if state == "LA"
...
eststo: qui reg frac_rev city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250917_issuerlvl_debtchoice_allgo_LALTGO.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and debt choice (All GO Vote)") star(* .10 ** .05 *** .01) ///
	s(N r2_a SE, fmt(%9.0fc 3) label ("N" "Adj. $ R^2$" "Cluster")) ///
	/*drop(ln_gdp ln_pers_inc ln_percap_inc ln_emp _cons)*/ label booktabs noobs nonotes nogaps ///
	order(city_go_vote $countydemo glm_proactive state_ltgo_allowed state_go_vote)
...
eststo: qui reg issuer_yield_spread city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250917_issuerlvl_yieldspread_allgo.tex", replace t noconstant b(3) ///
...
------------+-----------------------------------
       ltgo |     10,315       15.93       15.93
        rev |     33,472       51.71       67.64
       utgo |     20,946       32.36      100.00
------------+-----------------------------------
      Total |     64,733      100.00
*/
