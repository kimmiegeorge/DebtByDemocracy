...
import delimited using "$NC\school election data\260312_county_vote_history.csv", clear varn(1)

*save file
save "$NC\school election data\260312_county_vote_history.dta", replace
...
*Merge in county vote history
use "$NC\260310_countyyr_elec_demo_educ.dta", clear
mmerge county year using "$NC\school election data\260312_county_vote_history.dta", type(1:1) missing(nomatch)
drop _merge
sort county year
...
*save this
save "$NC\260312_countyyr_elec_demo_educ_votehist.dta", replace
...
reg partisan_ever region_id avg_vote_margin_lagelec avg_cand_lagelec n_close_lagelec ///
	us_house_pct_rep_lagelec pct_female_lg1 pct_18over_lg1 pct_65over_lg1 ///
...
- Counties are more likely to be partisan if:
	- Further east region
	- Lower vote margin (more competitive) but fewer candidates (less competitive)
	- More male
	- Higher % under 18
	- Higher % of over 65
	- Higher % White
	...
	- Lower state funds for schools
	- Higher spend per student
	
- Characterisics that are unrelated:
	- Num close elections
	- Prior House election voting behavior
	- Funds to schools
...
reg partisan_ever region_id avg_vote_margin_lagelec avg_cand_lagelec n_close_lagelec ///
	...
	if switch_pre2010 == 0 ///
	, vce(robust)
*Much weaker on the extensive margin, higher R2
...
*Run Cox only within counties that adopt post-2010
preserve

drop if partisan_ever == 0
stset t_exit, id(fips) failure(switch==1) enter(t_enter) origin(time 2010) exit(time 2025)

eststo clear
eststo: stcox avg_vote_margin_lagelec avg_cand_lagelec us_house_pct_rep_lagelec ln_gdp_lg1 ln_pop_lg1 pct_white_lg1 ln_spendpp_tt_lg1 ///
	, cluster(fips)
	estadd local SE "County"
esttab using "$DESCRIPT/260312_cox_intensive.tex", replace eform t noconstant b(3) ///
	title("Cox hazard model for treatment timing (only counties adopting post-2010)") star(* .10 ** .05 *** .01) label booktabs noobs nonotes nogaps ///
	s(N SE, fmt(%9.0fc 3) label ("N" "Cluster"))
*Similar story 
/*
Cox regression with Breslow method for ties

No. of subjects =  34                                   Number of obs =    223
No. of failures =  32
Time at risk    = 223
                                                        Wald chi2(7)  =  16.97
Log pseudolikelihood = -108.77074                       Prob > chi2   = 0.0176

                                              (Std. err. adjusted for 34 clusters in fips)
------------------------------------------------------------------------------------------
                         |               Robust
                      _t | Haz. ratio   std. err.      z    P>|z|     [95% conf. interval]
-------------------------+----------------------------------------------------------------
 avg_vote_margin_lagelec |   4.098241   2.377608     2.43   0.015     1.314529    12.77688
        avg_cand_lagelec |   1.132812   .0583358     2.42   0.015     1.024057    1.253117
us_house_pct_rep_lagelec |   .8261592   .6904002    -0.23   0.819     .1605966    4.250022
              ln_gdp_lg1 |   1.240707   .2329626     1.15   0.251     .8587025    1.792652
              ln_pop_lg1 |   .6216083   .1741586    -1.70   0.090     .3589484    1.076469
           pct_white_lg1 |   .7741074   .5890574    -0.34   0.737     .1742124    3.439722
       ln_spendpp_tt_lg1 |   .3534362   .3088364    -1.19   0.234     .0637561    1.959297
------------------------------------------------------------------------------------------
*/
...
--------------------------------------------------------
             |        rho     chi2       df    Prob>chi2
-------------+------------------------------------------
avg_vote_m~c |    0.28092     1.00        1       0.3174
avg_cand_l~c |    0.37333     1.81        1       0.1782
us_house_p~c |    0.16148     0.15        1       0.6985
  ln_gdp_lg1 |   -0.25282     0.32        1       0.5701
  ln_pop_lg1 |    0.22167     0.25        1       0.6140
pct_white_~1 |    0.00780     0.00        1       0.9862
ln_spendpp~1 |    0.14634     0.15        1       0.6994
-------------+------------------------------------------
 Global test |                1.94        7       0.9633
--------------------------------------------------------
*/
...
- ln_gdp / 0.007 / 0.02 / 0.980
- ln_pop / -0.008 / -0.04 / 0.970
- ln_pers_inc / 0.043 / 0.18 / 0.856
- ln_funds_federal_total / -0.058 / -0.27 / 0.784
- ln_funds_state_total / 0.011 / 0.05 / 0.957
- ln_funds_local_total / 0.068 / 0.29 / 0.774
- ln_funds_tt / 0.008 / 0.04 / 0.968
- ln_spendpp_tt / -0.006 / -0.17 / 0.868
*/
