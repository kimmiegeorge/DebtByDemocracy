...
***Import BEA data for states**
import delimited "$BEA\SASUMMARY__ALL_AREAS_1998_2023.csv", varnames(1) clear
*only keep 2001 and after to align with county data
drop v9-v11
rename v12 year2001
rename v13 year2002
rename v14 year2003
...
*only keep real GDP, personal income, per capita personal income, total employment
keep if inlist(linecode,1,5,10,15)
drop region tablename industryclassification
drop unit description
drop geofips
...
keep if linecode == 1
*reshape
reshape long year, i(state_name) j(yr)
rename year state_gdp
rename yr year
drop linecode
label var state_gdp "State GDP"
save "$BEA\state_gdp_2001_2022.dta", replace
restore 
...
*reshape
reshape long year, i(state_name) j(yr)
rename year state_persinc
rename yr year
drop linecode
label var state_persinc "State pers inc"
save "$BEA\state_persinc_2001_2022.dta", replace
restore 
...
*reshape
reshape long year, i(state_name) j(yr)
rename year state_percap_inc
rename yr year
drop linecode
label var state_percap_inc "State per capita inc"
save "$BEA\state_percap_inc_2001_2022.dta", replace
restore 
...
*reshape
reshape long year, i(state_name) j(yr)
rename year state_employment
rename yr year
drop linecode
label var state_employment "State employment"
save "$BEA\state_employment_2001_2022.dta", replace
restore 