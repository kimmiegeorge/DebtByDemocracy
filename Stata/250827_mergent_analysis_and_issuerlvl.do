...
eststo: qui reg frac_rev city_go_vote $countydemo ln_county_debt_other ///
	...
	if insample_utgo_only == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250827_issuerlvl_debtchoice_utgoonly.tex", replace t noconstant b(3) ///
	...
	order(city_go_vote $countydemo ln_county_debt_other glm_proactive state_ltgo_allowed state_go_vote)
...
eststo: qui reg frac_rev city_go_vote $countydemo ln_county_debt_other ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250827_issuerlvl_debtchoice_allgo.tex", replace t noconstant b(3) ///
...
eststo: qui reg issuer_yield_spread city_go_vote $countydemo ln_county_debt_other ///
	...
	if insample_utgo_only == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250827_issuerlvl_yieldspread_utgoonly.tex", replace t noconstant b(3) ///
...
eststo: qui reg issuer_yield_spread city_go_vote $countydemo ln_county_debt_other ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250827_issuerlvl_yieldspread_allgo.tex", replace t noconstant b(3) ///
...
eststo: qui reghdfe offering_yield_tr city_go_vote ln_amount_tr ln_maturity_tr ///
	...
	estadd local purposeFE "Yes"
	estadd local SE "Issue"	
esttab using "$RESULTS/250827_bondlvl_yield.tex", ///
