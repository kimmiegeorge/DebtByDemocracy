...
eststo clear
eststo: estpost sum city_go_vote offering_yield_tr ln_amount ln_maturity callable sinkable insured rated ///
	...
	if city_rev_vote == 0 & city_go_vote != . & go_unlim == 1 & offering_yield_tr != . , d
esttab using "$DESCRIPT\Bondlevel\250312_bondlevel_sumstats_city.tex", replace ///
...
sum amount if city_rev_vote == 0 & city_go_vote != . & go_unlim == 1 & offering_yield_tr != .
...
*Show flexes for state controls and county controls 
...
esttab using "$RESULTS/250311_city_yield_UTGO.tex", replace t noconstant b(3) ///
