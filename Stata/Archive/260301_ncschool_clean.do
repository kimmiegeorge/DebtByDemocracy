...
import delimited "$NC\NC_schoolboard_elections.csv", varnames(1) case(lower) clear
...
save "$NC\260301_schoolboard_electionlevel_clean.dta", replace

**# Bookmark #3
*Descriptives at the election level:
use "$NC\260301_schoolboard_electionlevel_clean.dta", clear
tab partisan
/*
      (max) |
   partisan |      Freq.     Percent        Cum.
...
gcollapse (max) partisan_yr n_races avg_vote_margin avg_cand avg_oneparty ///
...
*save file
save "$NC\260301_schoolboard_countyyrlevel.dta", replace
...
gcollapse (max) partisan_yr year_to_partisan county_switch switch_pre2010  ///
	, by(county year)	
export delimited using "$NC\260302_county_partisan_2010_2016_2018.csv", replace
restore
