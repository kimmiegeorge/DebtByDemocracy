...
global RESULTS "$MAIN\Results\2025-08_results"
...
label var city_go_vote "UTGO Vote"
...
eststo: qui reg frac_rev city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_utgo_only == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250829_issuerlvl_debtchoice_utgoonly.tex", replace t noconstant b(3) ///
...
label var city_go_vote "All GO Vote"
*Outcome var: % UTGO, % LTGO, % Rev*
*Cluster by county
eststo clear
eststo: qui reg frac_utgo city_go_vote ///
...
eststo: qui reg frac_rev city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250829_issuerlvl_debtchoice_allgo.tex", replace t noconstant b(3) ///
...
eststo: qui reg issuer_yield_spread city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_utgo_only == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250829_issuerlvl_yieldspread_utgoonly.tex", replace t noconstant b(3) ///
...
eststo: qui reg issuer_yield_spread city_go_vote $countydemo /*ln_county_debt_other*/ ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250829_issuerlvl_yieldspread_allgo.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and yield spread (All GO Vote)") star(* .10 ** .05 *** .01) ///
	s(N r2_a SE, fmt(%9.0fc 3) label ("N" "Adj. $ R^2$" "Cluster")) ///
	/*drop(ln_gdp ln_pers_inc ln_percap_inc ln_emp _cons)*/ label booktabs noobs nonotes nogaps ///
	order(city_go_vote $countydemo glm_proactive state_ltgo_allowed state_go_vote)

**Look at what happens for supermajority**
*gen indicator for supermajority
*states: CA, ID, MO, ND, OK, SD, WA, WV
gen city_go_vote_super = 1 if inlist(state,"CA", "ID", "MO", "ND", "OK", "SD", "WA", "WV") 	
replace city_go_vote_super = 0 if city_go_vote_super == . & city_go_vote != .
replace city_go_vote_super = 0 if city_go_vote == 0
tab city_go_vote_super
/*
city_go_vot |
    e_super |      Freq.     Percent        Cum.
------------+-----------------------------------
          0 |      2,879       82.83       82.83
          1 |        597       17.17      100.00
------------+-----------------------------------
      Total |      3,476      100.00
*/
tab city_go_vote city_go_vote_super
/*
    All GO |     Supermajority
      Vote |         0          1 |     Total
-----------+----------------------+----------
         0 |     1,059          0 |     1,059 
         1 |     1,820        597 |     2,417 
-----------+----------------------+----------
     Total |     2,879        597 |     3,476 
*/

*to run XS, use numeric id for fips
gegen fips_num = group(fips)

*Only do the XS for All GO Vote only where there are enough states
label var city_go_vote "All GO Vote"
label var city_go_vote_super "Supermajority"
*Outcome var: % UTGO, % LTGO, % Rev*
*Cluster by county
eststo clear
eststo: qui reghdfe frac_utgo i.city_go_vote_super##i.(city_go_vote glm_proactive state_ltgo_allowed state_go_vote) ///
	i.city_go_vote_super##c.($countydemo)  ///
	if insample_allgo == 1, vce(cluster i.fips_num##i.city_go_vote_super)
	estadd local SE "County"
*% LTGO
eststo: qui reghdfe frac_ltgo i.city_go_vote_super##i.(city_go_vote glm_proactive state_ltgo_allowed state_go_vote) ///
	i.city_go_vote_super##c.($countydemo)  ///
	if insample_allgo == 1, vce(cluster i.fips_num##i.city_go_vote_super)
	estadd local SE "County"
*% Rev
eststo: qui reghdfe frac_rev i.city_go_vote_super##i.(city_go_vote glm_proactive state_ltgo_allowed state_go_vote) ///
	i.city_go_vote_super##c.($countydemo)  ///
	if insample_allgo == 1, vce(cluster i.fips_num##i.city_go_vote_super)
	estadd local SE "County"
esttab using "$RESULTS/250908_issuerlvl_debtchoice_allgo_supermajority.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and debt choice (All GO Vote), supermajority XS") star(* .10 ** .05 *** .01) ///
	s(N r2_a SE, fmt(%9.0fc 3) label ("N" "Adj. $ R^2$" "Cluster")) ///
	label booktabs noobs nonotes nogaps ///
	keep(1.city_go_vote#1.city_go_vote_super 1.city_go_vote 1.city_go_vote_super) ///
	order(1.city_go_vote#1.city_go_vote_super 1.city_go_vote 1.city_go_vote_super) ///
	varlabels(1.city_go_vote#1.city_go_vote_super "\rowcolor{ltblue} All GO Vote $ \times$ Supermajority")
	
reghdfe frac_utgo i.city_go_vote_super##i.(city_go_vote glm_proactive state_ltgo_allowed state_go_vote) ///
	i.city_go_vote_super##c.($countydemo)  ///
	if insample_allgo == 1, vce(cluster i.fips_num##i.city_go_vote_super)	
	
reg frac_utgo city_go_vote_super $countydemo /*ln_county_debt_other*/ ///
	glm_proactive state_ltgo_allowed state_go_vote ///
	if insample_allgo == 1, vce(cluster fips)

reg frac_utgo city_go_vote_super	if insample_allgo == 1
...
eststo: qui reghdfe offering_yield_tr city_go_vote ln_amount_tr ln_maturity_tr ///
	...
	estadd local purposeFE "Yes"
	estadd local SE "Issue"	
esttab using "$RESULTS/250829_bondlvl_yield.tex", ///
...
reghdfe offering_yield_tr city_go_vote ln_amount_tr ln_maturity_tr ///
	callable sinkable insured rated ///
	$countydemo glm_proactive state_ltgo_allowed state_go_vote  ///
	if city_rev_vote == 0 & go_unlim == 1 & temp1 != 1  ///
	, absorb(yrmonth purp_broad_id) vce(cluster issue_id)
*t-stat = -1.50
...
gen temp2 = 1 if city_go_vote == 1 & city_rev_vote == 1
replace temp2 = 1 city_go_vote == 1 & city_rev_vote == .
replace temp2 = 1 if city_go_vote == 1 & city_rev_vote == 0
replace temp2 = 1 if city_go_vote == 0 & city_rev_vote == 0

reghdfe offering_yield_tr city_go_vote ln_amount_tr ln_maturity_tr ///
	callable sinkable insured rated ///
	$countydemo glm_proactive state_ltgo_allowed state_go_vote  ///
	/*city_rev_vote*/ ///
	if go_unlim == 1 & temp2 == 1 ///
	, absorb(yrmonth purp_broad_id) vce(cluster issue_id)	
...
gen temp3 = 1 if inlist(state,"NV","KS","MN","IA","IL","IN","SC")	
replace temp3 = 1 if inlist(state,"VA","PA","NY","MD","NY","CT")	
count if temp3 == 1
...
reghdfe offering_yield_tr i.city_go_vote##i.go_unlim ln_amount_tr ln_maturity_tr ///
	callable sinkable insured rated ///
	$countydemo glm_proactive state_ltgo_allowed state_go_vote  ///
	/*city_rev_vote*/ ///
	if city_rev_vote == 0 ///
