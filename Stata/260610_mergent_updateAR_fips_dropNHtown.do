...
/*
- Two updates: 
	- Switch Arkansas revenue bond to "depends"
	- Identify and drop NH towns. Drop NH bond banks
...
*Make Arkansas change
replace city_rev_vote = . if state == "AR"
...
*Check duplicate seed issuers that KM flagged
...
/*
         ...
         ------------+---------------------------------------------------------
              _merge |   1504  obs only in master data                (code==1)
                     |  71519  obs only in using data                 (code==2)
                     | 325231  obs both in master and using data      (code==3)
-------------------------------------------------------------------------------
*/
*br if _merge == 1
...
save "$MERGENT\Clean\260610_city_cusiplevel_statereq_purpose_yieldspread.dta", replace
...
*Implement AR, NH cleaning above
replace city_rev_vote = . if state == "AR"
...
*save file
save "$MERGENT\Clean\260610_city_issuerlevel.dta", replace
...
use "$MERGENT\Clean\260610_city_issuerlevel.dta", clear
...
*save
save "$MERGENT\Clean\260610_city_issuerlevel_yieldspread.dta", replace
