...
- Add yield spread to issuer-level data
*/
...
**# Bookmark #1
**Make issuer-level yield spread and merge into main issuer-level data**

*Start with main city bond-level data*
use "$MERGENT\Clean\250624_city_cusiplevel_statereq_purpose.dta", clear
*328,201 obs

*br state seed_issuer seed_issuer_id issue_id cusip

*merge in yield spreads from KM
mmerge issue_id cusip using "$MERGENT\Clean\250603_bond_level_off_yield_spread.dta", ///
	type(1:1) missing(nomatch)
/*
                 obs | 722207
                vars |    231  (including _merge)
         ------------+---------------------------------------------------------
              _merge |   5000  obs only in master data                (code==1)
                     | 394006  obs only in using data                 (code==2)
                     | 323201  obs both in master and using data      (code==3)
-------------------------------------------------------------------------------
*/
*_merge == 2 for counties, schools
drop if _merge == 2
br if _merge == 1
sort state seed_issuer issue_id offering_date
*not clear why these 5,000 unmatched
*save these separately
preserve
keep if _merge == 1
keep state seed_issuer seed_issuer_id issue_id issue_description cusip amount
save "$MERGENT\Clean\250630_cityyieldspread_unmatched.dta", replace
restore
...
*save these to ask KM later
preserve
keep if issuer_yield_spread == 0
save "$MERGENT\Clean\250630_issuers_yieldspread_unmatched.dta", replace
restore
...
*save file
save "$MERGENT\Clean\250630_issuers_yieldspread.dta", replace

*Add this to main issuer-level file
use "$MERGENT\Clean\250625_city_issuerlevel.dta", clear
mmerge seed_issuer using "$MERGENT\Clean\250630_issuers_yieldspread.dta", ///
	type(n:1) missing(nomatch)
*all matched, hooray
drop _merge
sort state seed_issuer
...
*save file
save "$MERGENT\Clean\250625_city_issuerlevel_yieldspread.dta", replace