...
reghdfe go_unlim city_go_vote state_go_vote state_ltgo_allowed ///
	state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien ///
	ln_gdp ln_pers_inc ln_percap_inc ln_emp ///
	if city_rev_vote == 0  ///
	, absorb(yrmonth) vce(cluster state yrmonth)
...
reghdfe rev city_go_vote state_go_vote state_ltgo_allowed ///
	state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien ///
	ln_gdp ln_pers_inc ln_percap_inc ln_emp ///
	if city_rev_vote == 0 & utgo_only != 1  ///
	, absorb(year) vce(cluster state)
*coeff = 0.063, p=0.684

reg rev city_go_vote if city_rev_vote == 0 & utgo_only != 1 
*coeff = 0.232, p=0

reg rev city_go_vote state_go_vote state_ltgo_allowed state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien if city_rev_vote == 0 & utgo_only != 1 
*coeff = 0.036, p=0

reg rev city_go_vote state_go_vote state_ltgo_allowed state_fullfaith ///
	state_sep_debtservice_levy state_sep_pledgerev state_statutorylien ///
	ln_gdp ln_pers_inc ln_percap_inc ln_emp ///
	if city_rev_vote == 0 & utgo_only != 1 
*coeff = 0.023, p=0

reghdfe rev city_go_vote state_go_vote state_ltgo_allowed state_fullfaith ///
	state_sep_debtservice_levy state_sep_pledgerev state_statutorylien ///
	ln_gdp ln_pers_inc ln_percap_inc ln_emp ///
	if city_rev_vote == 0 & utgo_only != 1 ///
	, absorb(year)
...
*There's a sense in which this should be done at the issuance-level. Within an issuance, there aren't going to be rev and GO bonds
gegen n_bond_issue = count(amount), by(issue_id)
sum n_bond_issue if rev == 1
*mean is 17
sum n_bond_issue if go_unlim == 1
*mean is 16
sum n_bond_issue if go_lim == 1
*mean is 17
