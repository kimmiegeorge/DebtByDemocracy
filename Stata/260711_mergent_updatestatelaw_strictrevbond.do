...
/*Multiple updates:
	- Maine: Towns need GO and rev vote; cities don't, but can add a GO vote through charter
		- Gen indicator for Maine town vs. city
	- New Hampshire: Bring back NH towns and use indicator for NH town vs. city
...
mmerge state using "$DATA\Bond Elections\260709_election_requirements_by_state.dta", ///
...
bond_type_n |
         ew |      Freq.     Percent        Cum.
------------+-----------------------------------
         go |    238,950       78.32       78.32
        rev |     66,153       21.68      100.00
------------+-----------------------------------
      Total |    305,103      100.00
*/
...
*save file
save "$MERGENT\Clean\260709_city_cusiplevel_statereq_purpose_yieldspread.dta", replace

*Get list of issuances in ME, NH to hand-check. Only need to run this once*
use "$MERGENT\Clean\260709_city_cusiplevel_statereq_purpose_yieldspread.dta", clear
keep if inlist(state,"ME","NH")
keep if go_unlim == 1
*Before getting to issuance level, keep a cusip for manual look-up
gegen temp1 = max(maturity_mths), by(issue_id)
count if temp1 == .
gen temp2 = cusip if maturity_mths == temp1
keep if maturity_mths == temp1
drop temp1
*None missing, good
*Get to issuance level
keep state seed_issuer seed_issuer_id issuer_long_name year offering_date issue_id issue_description cusip temp2 me_city nh_city
drop issuer_long_name temp2
*Only need to check ME cities and NH towns
gen temp1 = 1 if me_city == 1
replace temp1 = 1 if nh_city == 0
keep if temp1 == 1
*103 issuances
drop temp1

*save excel
export excel "$DATA\Other\260711_check_me_nh.xlsx", firstrow(var) replace
...
*5,617 issuers
*save file
save "$MERGENT\Clean\260709_issuers_yieldspread.dta", replace
...
*save file
save "$MERGENT\Clean\260709_city_issuerlevel.dta", replace
...
use "$MERGENT\Clean\260709_city_issuerlevel.dta", clear
mmerge seed_issuer using "$MERGENT\Clean\260709_issuers_yieldspread.dta", ///
	type(n:1) missing(nomatch)
*all matched, hooray
drop _merge
sort state seed_issuer 
...
label var nh_city "NH City"
label var me_city "ME City"

*save
save "$MERGENT\Clean\260709_city_issuerlevel_yieldspread.dta", replace
