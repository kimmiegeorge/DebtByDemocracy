...
preserve
gcollapse (sum) amount num_cusip, by(issuer_type)
list
restore

sum amount if issuer_type == "school"
sum amount if issuer_type == "city"

preserve
keep if issuer_type != "county"
gcollapse (sum) amount num_cusip, by(issuer_type year)
sum amount if issuer_type == "city"
sum amount if issuer_type == "school"
restore
...
*number of issuers
gunique seed_issuer
*N = 332,624; 5,953 unbalanced groups of sizes 1 to 1,681
count if go_unlim == 1 
*201,182
...
***Fix naming and some coding of whether state GO debt requires voter approval***
...
preserve
gcollapse (count) temp1 = year, by(state state_go_vote)
drop temp1
list
restore
...
esttab using "$DESCRIPT\Bondlevel\250313_bondlevel_sumstats_city_v3.tex", replace ///
...
esttab using "$RESULTS/250313_city_yield_UTGO_vF v1.tex", replace t noconstant b(3) ///
...
reghdfe offering_yield_tr city_go_vote ln_amount_tr ln_maturity_tr callable sinkable insured rated ///
	state_godebt_limit state_ltgo_allowed state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien ln_state_gdp ///
	ln_gdp ln_pers_inc ln_percap_inc ln_emp ///
	if city_rev_vote == 0 & go_unlim == 1  ///
	, absorb(yrmonth purp_broad_id) vce(cluster state)	
...
esttab using "$RESULTS/250313_city_yield_UTGO_nostatelimit.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and offering yield") star(* .10 ** .05 *** .01) ///
...
pwcorr offering_yield_tr state_godebt_limit state_ltgo_allowed state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien, star(0.01)

preserve
gcollapse (count) temp1 = year, by(state state_godebt_limit state_ltgo_allowed state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien)
pwcorr state_godebt_limit state_ltgo_allowed state_fullfaith state_sep_debtservice_levy state_sep_pledgerev state_statutorylien, star(0.1)
*count
restore
...
*no trim
eststo clear
eststo: qui reghdfe offering_yield city_go_vote ln_amount ln_maturity_mths callable sinkable insured rated ///
	if city_rev_vote == 0 & go_unlim == 1 ///
	, absorb(yrmonth purp_broad_id) vce(cluster state)
	estadd local yrmonthFE "Yes"
	estadd local purposeFE "Yes"
...
esttab using "$RESULTS/250313_city_yield_UTGO_notrim.tex", replace t noconstant b(3) ///
	title("GO bond referendum requirements and offering yield") star(* .10 ** .05 *** .01) ///
	s(N r2_a yrmonthFE purposeFE bondcon statecon countycon SE, fmt(%9.0fc 3) label ("N" "Adj. $ R^2$" "Year-Month FE" "Purpose FE" "Bond Controls" "State Controls" "County Controls" "Cluster")) ///
	/*drop(ln_gdp ln_pers_inc ln_percap_inc ln_emp _cons)*/ label booktabs noobs nonotes 
