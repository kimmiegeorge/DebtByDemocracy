...
use "$MERGENT\Clean\250630_city_issuerlevel_yieldspread.dta", clear
...
eststo: qui reg issuer_yield_spread city_go_vote ///
	...
	if insample_allgo == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250630_allgo_yieldspread_v1.tex", replace t noconstant b(3) ///
...
eststo: qui reg issuer_yield_spread  city_go_vote ///
	...
	if insample_utgo_only == 1, vce(cluster fips)
	estadd local SE "County"
esttab using "$RESULTS/250630_utgoonly_yieldspread_v1.tex", replace t noconstant b(3) ///
