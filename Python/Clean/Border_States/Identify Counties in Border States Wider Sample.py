...
MERGENT_DATA_FILE = '260716_city_cusiplevel_statereq_purpose_yieldspread.dta'
COUNTY_LATLNG_FILE = 'us_county_latlng.csv'
RP_ISSUANCE_FILE = 'Issuance_Lvl_News_With_Lagged_News.csv'
RP_CITY_MONTH_FILE = 'Full_City_Month_Data_Headline_Filter.gzip'
...
rp_issuance = pl.read_csv(f'{clean_data_dir}/News/{RP_ISSUANCE_FILE}')
...
#                  .join(border_issuer_groups, on = 'seed_issuer_id', how = 'inner')
#                  )
# rp_city_month.write_csv(f'{clean_data_dir}/Border States/Border Matches RP City Month Lvl Expanded Set Buffer {BUFFER_DISTANCE}.csv')
...
#                .filter(pl.col('seed_issuer_id').is_in(border_issuer_ids))
#                .join(border_issuer_groups, on = 'seed_issuer_id', how = 'inner'))
# issuance.write_csv(f'{clean_data_dir}/Border States/Border Matches Secondary Issuance Lvl Expanded Set Buffer {BUFFER_DISTANCE}.csv')


#%%
# just save list of issuers
#issuers = mergent_border_matches.select(['seed_issuer_id', 'seed_issuer', 'group', 'category']).unique()
#issuers.write_csv(f'{clean_data_dir}/Border States/Border Matches Issuers.csv')
...
#            .unique()
#            )
# issuers.write_csv(f'{clean_data_dir}/Border States/Border States GO Unlim Blue Only Issuers Expanded Set Buffer {BUFFER_DISTANCE}.csv')
