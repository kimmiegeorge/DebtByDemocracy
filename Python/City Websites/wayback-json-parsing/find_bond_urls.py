'''
find websites with URLs metioning bonds
'''
import os.path
...
def wayback_query(host, match_type="exact", collapse_time=10) -> pd.DataFrame | None:
    # Wayback API query
...
def query_filter(df, freq, date_range=None):
    ...
    df_r["period"] = df_r["agg_datetime"].dt.to_period("Q")
    ...
    while len(urls_dict) > 0 and len(tree_res) < max_url and sub <= max_sub:
        ...
        for sub_url in url_res["subURLs"]:
            ...
            if priority_score > 0:
                ...
                end_time = time.time()
                print(
                    "URL %s finished scraping with bond found in %.2f seconds. "
                    % (
                        url,
                        end_time - start_time
                    )
                )
                return host, sub_url, 1
...
def WaybackScraper(
    ...
    for url in df["url"].values:
        _host, sub_url, has_bond = tree_scrape(url, host, max_url, max_sub)
        url_res_all[url] = [_host, sub_url, has_bond]
        time.sleep(6)

    return url_res_all

# %% Loop through cities and look for bonds
if __name__ == "__main__":
    out_fp = os.path.expanduser('~/Dropbox/Voting on Bonds/Data/Websites/Bond_Search_20250217')

    city_websites = ['www.bellairetx.gov', 'www.cstx.gov', 'www.bryantx.gov',
                     'www.springfield.il.us', 'www.rockfordil.gov',
                     'www.amarillo.gov', 'www.champaignil.gov',
                     'www.naperville.il.us', 'www.dallascityhall.com']
    ...
    '''
    city_websites = ['www.bellairetx.gov', 'www.cstx.gov']
    ...
    for city in city_websites:
        output_dict = WaybackScraper(city, freq='Q', date_range = date_range)
        df = pd.DataFrame(list(output_dict.values()), index=output_dict.keys(), columns=['Website', 'URL', 'Has_Bond'])
        df.to_csv(f'{out_fp}/{city}_output.csv', index=False)
