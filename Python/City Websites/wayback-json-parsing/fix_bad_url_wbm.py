...
if __name__ == "__main__":
    ...
    for year in range(2020, 1999, -1):
        urls = urls.filter(pl.col('year').eq(year))
        url_list = urls['original_url'].to_list()
        files = os.listdir(f'{PATH}Bad URL Annual Files/JSON_{year}/')
        ...
        for url in urls_to_process:
            WaybackScraper(host = url, freq = 'Y', date_range = date_range, path = f'{PATH}Bad URL Annual Files/JSON_{year}/')
