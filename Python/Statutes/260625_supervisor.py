#!/usr/bin/env python3
r"""
260625_supervisor.py  --  Debt by Democracy: state statute AI task

Detached-friendly supervisor for 260625_download_statutes.py (FINAL-14 Phase 2c
download: CT DE IL IN IA KS MD MN NV NY PA RI SC VA). One process that handles BOTH
failure modes so there is only one thing to keep alive, launched via the Windows Task
Scheduler so it survives the Claude desktop app/session being recycled (which reaps
bash-spawned background jobs as a group -- the confirmed root cause from the pilot).
...
MAX_ATTEMPTS. The cache makes every restart cheap (already-fetched pages skipped). On
a clean pass it self-unregisters the scheduled task.

NOTE on Cloudflare: the downloader fetches via 260625_justia_fetch.py, which PARKS on a
challenge (no crash) until the Firefox harvester writes a fresh cf_clearance -- printing
every ~45s so the stall watchdog stays happy. So a cookie expiry just PAUSES the crawl;
the supervisor here only really handles native crashes/hangs. The fast-fail cooldown is
kept defensively for the rare non-challenge transient that survives the in-fetch wait.

Run (normally via Task Scheduler; see the Register-ScheduledTask launch in chat):
  ...\venv\Scripts\python.exe -u ...\260625_supervisor.py
"""
...
PY = sys.executable
DOWNLOADER = HERE / "260625_download_statutes.py"
DOCS = (Path(r"C:\Users\juneh\Dropbox (Personal)\Voting on Bonds")
        / "Data" / "Statutes" / "documentation")
LOG = DOCS / "260625_download_log.txt"
SLOG = DOCS / "260625_supervisor_log.txt"
...
# Escalating backoff (seconds) for FAST failures (the rare non-challenge block).
COOLDOWNS = [300, 600, 1200, 1800, 1800]
TASK_NAME = "DbD_Final15Download"
...
def run_once() -> tuple[str, int | None, float]:
    ...
    fh.flush()
    p = subprocess.Popen([PY, "-u", str(DOWNLOADER), "--download", "--delay", DELAY],
...
def main() -> None:
    ...
    slog(f"supervisor start (stall {STALL_MINUTES} min, poll {POLL_S}s, "
         f"max {MAX_ATTEMPTS} attempts, delay {DELAY}s, escalating cooldown {COOLDOWNS})")
    fail_streak = 0
    try:
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            # mid-crawl) -> resume fast. A FAST failure is the rare non-challenge block
            # signature -> ride it out with escalating backoff.
            if elapsed >= PROGRESS_MIN_S:
                ...
                fail_streak += 1
                slog(f"fast fail -> likely transient block; cooldown {cd}s (backing off)")
            time.sleep(cd)
