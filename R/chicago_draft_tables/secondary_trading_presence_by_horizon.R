...
r1 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_1yr[year >=2005], vcov = ~issue_id)
summary(r1)
r2 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_1yr[year >= 2005], vcov = ~issue_id )
summary(r2)

r3 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_3yr[year >=2005], vcov = ~issue_id)
summary(r3)
r4 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_3yr[year >= 2005], vcov = ~issue_id )
summary(r4)

r5 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_5yr[year >=2005], vcov = ~issue_id)
summary(r5)
r6 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_5yr[year >= 2005], vcov = ~issue_id )
summary(r6)


r7 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_allyr[year >=2005], vcov = ~issue_id)
summary(r7)
r8 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad, data = data_allyr[year >= 2005], vcov = ~issue_id )
summary(r8)
...
writeLines(modified_output, paste0(tables_wd, '/liquidity_options/full_sample_bond_level_trade_activity_time_horizons.tex'))
...
r1 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_1yr[year >=2005], vcov = ~issue_id)
summary(r1)
r2 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_1yr[year >= 2005], vcov = ~issue_id )
summary(r2)

r3 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_3yr[year >=2005], vcov = ~issue_id)
summary(r3)
r4 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_3yr[year >= 2005], vcov = ~issue_id )
summary(r4)

r5 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_5yr[year >=2005], vcov = ~issue_id)
summary(r5)
r6 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_5yr[year >= 2005], vcov = ~issue_id )
summary(r6)


r7 <- feols(secondary_trade ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_allyr[year >=2005], vcov = ~issue_id)
summary(r7)
r8 <- feols(log_amount ~ city_go_vote + ln_amount + ln_maturity_mths + callable  + sinkable + insured + rated + log_pop + log_gdp + log_pers_inc + log_employment|year + purp_broad + group, data = border_states_allyr[year >= 2005], vcov = ~issue_id )
summary(r8)
...
modified_output <- modify_etable_rounding(table_call, coef_digits = 3, tstat_digits = 2)
writeLines(modified_output, paste0(tables_wd, '/liquidity_options/border_state_bond_level_trade_activity_time_horizons.tex'))
