...
data <- fread('~/Dropbox/Voting on Bonds/Data/MSRB/Processed/Trade_Level_Regression_Dataset_251202.csv')
...
cd <- cd[, .(issue_id, num_disclosures_within_1_year, num_disclosures_within_3_years,
             num_disclosures_within_5_years, num_disclosures_full_msrb)]
...
# 1 yr
r1 <- feols(markup ~ city_go_vote+ treasury_10yr + num_disclosures_within_1_year + rating_low +
              ...
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == 1 & as.integer(trade_date - offering_date) <= 365], vcov = ~cusip + year)
summary(r1)

r2<- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_1_year +rating_low +
             log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
             callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == -1 & as.integer(trade_date - offering_date) <= 365], vcov = ~cusip + year)
summary(r2)

# 3 yr
r3 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_3_years +rating_low +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == 1 & as.integer(trade_date - offering_date) <= 3*365], vcov = ~cusip + year)
summary(r3)

r4<- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_3_years +rating_low +
             log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
             callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == -1 & as.integer(trade_date - offering_date) <= 3*365], vcov = ~cusip + year)
summary(r4)

# 5 yr
r5 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_5_years +rating_low +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == 1 & as.integer(trade_date - offering_date) <= 5*365], vcov = ~cusip + year)
summary(r5)

r6 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_5_years +rating_low +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == -1 & as.integer(trade_date - offering_date) <= 5*365], vcov = ~cusip + year)
summary(r6)

# full
r7 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_full_msrb +rating_low +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == 1], vcov = ~cusip + year)
summary(r7)

r8 <- feols(markup ~ city_go_vote+ treasury_10yr + num_disclosures_full_msrb +rating_low +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad, data = data[trade_sign == -1], vcov = ~cusip + year)
summary(r8)
...
writeLines(modified_output, paste0(tables_wd, '/liquidity_options/cd_control_trade_level_transaction_costs_by_time_horizon.tex'))
...
# 1 yr
r1 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_1_year +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == 1 & as.integer(trade_date - offering_date) <= 365], vcov = ~cusip + year)
summary(r1)

r2 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_1_year +
            ...
            + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == -1 & as.integer(trade_date - offering_date) <= 365], vcov = ~cusip + year)
summary(r2)

# 3 yr
r3 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_3_years +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == 1 & as.integer(trade_date - offering_date) <= 3*365], vcov = ~cusip + year)
summary(r3)

r4 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_3_years +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + callable 
            + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == -1 & as.integer(trade_date - offering_date) <= 3*365], vcov = ~cusip + year)
summary(r4)


# 5 yr
r5 <- feols(markup ~ city_go_vote+ treasury_10yr ++num_disclosures_within_5_years +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == 1 & as.integer(trade_date - offering_date) <= 5*365], vcov = ~cusip + year)
summary(r5)

r6 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_within_5_years +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + callable 
            + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == -1 & as.integer(trade_date - offering_date) <= 5*365], vcov = ~cusip + year)
summary(r6)


# full
r7 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_full_msrb +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + 
              callable + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == 1], vcov = ~cusip + year)
summary(r7)

r8 <- feols(markup ~ city_go_vote+ treasury_10yr +num_disclosures_full_msrb +
              log_trade_size + log_daily_par_volume + bond_age_years + time_to_maturity_years + inventory_indicator + ln_amount + ln_maturity_mths + rated + sinkable + insured + callable 
            + log_gdp + log_pop + log_pers_inc + log_employment|year + purp_broad + group, data = border_states[trade_sign == -1], vcov = ~cusip + year)
summary(r8)
...
modified_output <- modify_etable_rounding(table_call, coef_digits = 3, tstat_digits = 2)
writeLines(modified_output, paste0(tables_wd, '/liquidity_options/cd_control_border_trade_level_transaction_costs_by_time_horizon.tex'))
