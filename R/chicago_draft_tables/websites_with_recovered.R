...
desc_col[, variable := c('Vote', 'Bond URL Count',  'Bond Count', 
                         'Fiscal URL Count', 'Fiscal Count', 'PDF Financial Docs', 'Num Issuances', 'State Fiscal Monitor')]
...
data[, year := as.integer(as.character(year))]
data <- turnout_full[data, on = .(fips, year)]

data[, bond_fiscal_count := bond_count + fiscal_count]

data[, high_turnout := ifelse(REG_VOTER_TURNOUT_PCT > quantile(REG_VOTER_TURNOUT_PCT, 2/3, na.rm = T), 1, 0)]
data[, low_turnout := ifelse(REG_VOTER_TURNOUT_PCT < quantile(REG_VOTER_TURNOUT_PCT, 1/3, na.rm = T), 1, 0)]
#data[, high_diversity := ifelse(diversity >quantile(diversity, 2/3, na.rm = T), 1, 0)]

# THIS ONE IS INTERESTING - EFFECT IS LOWER FOR HIGH TURNOUT
r0 <- fixest::fepois(bond_fiscal_count ~ city_go_vote*high_turnout + ln_cum_num_issues  +  ln_gdp + ln_pop +  
                       ln_pers_inc  + ln_emp + state_monitor  |group + year, data = data, cluster ~ fips)
summary(r0)




table_call <- etable(r0,
                     coefstat = 'tstat',
                     style.tex = style.tex(main = 'aer', fixef.suffix = ' FE', yesNo = "Yes"),
                     fitstat = c('n', 'pr2'), 
                     se.below = TRUE, 
                     ...
                     tex = TRUE,
                     fontsize = 'small',
                     dict = c(bond_fiscal_count ='Website Disclosure',
                              bond_count ='Bond Count',
                              fiscal_url = 'Fiscal URL Count',
                              fiscal_count = 'Fiscal Count',
                              liabil_count = 'Liabilities Count',
                              revenue_count = 'Revenue Count',
                              expense_count = 'Expense Count',
                              financial_pdf_urls = 'PDF Financial Docs',
                              city_go_vote = 'Vote',
...
writeLines(modified_output, paste0(tables_wd, '/websites_turnout_xs.tex'))


data[, hhi := (PARTISAN_INDEX_DEM^2 + PARTISAN_INDEX_REP^2)]
data[, high_reg := ifelse(REG_VOTERS_PCT > quantile(REG_VOTERS_PCT, 2/3, na.rm = T), 1, 0)]
data[, high_hhi := ifelse(hhi > quantile(hhi, 2/3, na.rm = T), 1, 0)]

# THIS ONE IS INTERESTING - EFFECT IS LOWER FOR HIGH TURNOUT
r_turn <- fixest::fepois(bond_fiscal_count ~ city_go_vote*high_turnout + ln_cum_num_issues  +  ln_gdp + ln_pop +  
                       ln_pers_inc  + ln_emp + state_monitor  |group + year, data = data, cluster ~ fips)
summary(r_turn)

r_reg <- fixest::fepois(bond_fiscal_count ~ city_go_vote*high_reg + ln_cum_num_issues  +  ln_gdp + ln_pop +  
                           ln_pers_inc  + ln_emp + state_monitor  |group + year, data = data, cluster ~ fips)
summary(r_reg)

r_hhi <- fixest::fepois(bond_fiscal_count ~ city_go_vote*high_hhi + ln_cum_num_issues  +  ln_gdp + ln_pop +  
                          ln_pers_inc  + ln_emp + state_monitor  |group + year, data = data, cluster ~ fips)
summary(r_hhi)
...
table_call <- fixest::etable(
  ...
  dict = c(
    ...
    high_hhi = "High Partisan Concentration",
    "city_go_vote:high_turnout"  = "Vote × High Turnout",
    "city_go_vote:high_reg"      = "Vote × High Registered Voters",
    "city_go_vote:high_hhi" = "Vote × High Partisan", 
...
modified_output <- modify_etable_rounding(table_call, coef_digits = 3, tstat_digits = 2)
writeLines(modified_output, paste0(tbl_dir, '/websites_all_voter_xs.tex'))
