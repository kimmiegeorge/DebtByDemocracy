...
coefficient_results <- rbindlist(lapply(names(models), function(model_name) {
  ...
  coefficient_row <- coeftable(model)['all_go_vote', ]
  data.table(
    model = model_name,
...
results_file <- file.path(
  processed_dir,
  'point_in_time_debt_choice_2017_allgo_vs_az_co_mo_sd_vt_results.csv'
)
fwrite(coefficient_results, results_file)

sample_composition <- analysis_sample[, .(
  observations = .N,
  issuers_with_positive_go_revenue_debt = sum(mergent_go_revenue_outstanding_debt > 0),
...
), by = .(all_go_vote, state)][order(all_go_vote, state)]

sample_file <- file.path(
  processed_dir,
  'point_in_time_debt_choice_2017_allgo_vs_az_co_mo_sd_vt_sample.csv'
)
