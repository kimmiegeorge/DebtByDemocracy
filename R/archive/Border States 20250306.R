...
data[, upper99 := quantile(offering_yield, 0.99)]
data[, lower1 := quantile(offering_yield, .01)]
data[, offering_yield_tr_2 := ifelse(offering_yield > upper99 | offering_yield < lower1, NA, offering_yield)]
...
states <- unique(data[, .(state, seed_issuer_id, state_godebt_limit, state_ltgo_allowed, state_fullfaith, state_sep_debtservice_levy, state_sep_pledgerev, state_statutorylien)])
issuance_lvl <- states[issuance_lvl, on = .(seed_issuer_id)]


#data <- data[seed_issuer_id %in% issuance_lvl$seed_issuer_id]

reg1 <- felm(offering_yield_tr_2 ~ city_go_vote|yrmonth + group|0|purp_broad + state, data = data[go_unlim == 1 & category != 'green'])
reg2 <- felm(offering_yield_tr_2 ~ city_go_vote + ln_amount + ln_maturity + callable  + sinkable + insured + rated|yrmonth + group + purp_broad|0|purp_broad + state, data = data[go_unlim == 1 & category != 'green'])
reg3 <- felm(offering_yield_tr_2 ~ city_go_vote + ln_amount + ln_maturity + callable  + sinkable + insured + rated + 
               state_godebt_limit + state_ltgo_allowed +state_fullfaith + state_sep_debtservice_levy + state_sep_pledgerev + state_statutorylien|yrmonth + group + purp_broad|0|purp_broad + state, data = data[go_unlim == 1 & category != 'green'])
reg4 <- felm(offering_yield_tr_2 ~ city_go_vote + ln_amount + ln_maturity + callable  + sinkable + insured + rated + 
               state_godebt_limit + state_ltgo_allowed +state_fullfaith + state_sep_debtservice_levy + state_sep_pledgerev + state_statutorylien +
               ln_gdp + ln_emp + ln_percap_inc + ln_pers_inc|yrmonth+ group + purp_broad|0|purp_broad + state, data = data[go_unlim == 1 & category != 'green'])
...
stargazer(reg2, reg3, reg4, 
          ...
          out = paste0(tables_wd, '/Border States Yields Unlim Only.tex'))
...
reg1 <- felm(issuance_month_total_articles ~ city_go_vote + ln_amount + ln_maturity + callable + sinkable + insured + rated|year + purp_broad + group|0|purp_broad + state, data = issuance_lvl[go_unlim_bond_issuance == 1 & category != 'green'])
reg2 <- felm(issuance_month_total_articles ~ city_go_vote + ln_amount + ln_maturity + callable + sinkable + insured + rated + 
               ln_gdp + ln_employment + ln_percap_inc + ln_pers_inc|year + group + purp_broad|0|purp_broad + state, data = issuance_lvl[go_unlim_bond_issuance == 1 & category != 'green'])
...
stargazer(reg1, reg2,
          ...
          out = paste0(tables_wd, '/Border States Articles Unlimited Only.tex'))


reg1 <- felm(issuance_month_total_articles ~ city_go_vote|year + group|0|seed_issuer_id, data = issuance_lvl[rev_bond_issuance == 0 & category != 'green' & category != 'dark blue'])
reg2 <- felm(issuance_month_total_articles ~ city_go_vote + ln_amount + ln_gdp + ln_pers_inc + ln_percap_inc + ln_employment|year + group + purp_broad|0|seed_issuer_id, data = issuance_lvl[rev_bond_issuance == 0 & category != 'green' & category != 'dark blue'])
