...
format_table <- function(tex, cluster_level = "FIPS") {
  ...
  # 1. Add rowcolor to Vote line and t-stat line
  ...
  # 5. Remove empty line between Pseudo R2 and fixed effects
  r2_idx <- grep("Pseudo R\\$\\^2\\$", lines)
  ...
  if (length(bottomrule_idx) > 0) {
    # Count the number of columns (count & symbols in any data row)
    sample_row <- lines[vote_idx]
...
modified_output <- format_table(modified_output, cluster_level = "FIPS")
