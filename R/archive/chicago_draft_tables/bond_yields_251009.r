...
desc <- full_data[rolling_sum_monthly_article_count_12 > 0, .(offering_yield, 
                   offering_yield_spread,
                 city_go_vote, high_articles_12_0, 
                 maturity_mths, callable, sinkable, insured, rated)]
...
r1 <- feols(offering_yield ~ city_go_vote*high_articles_12_0  + maturity_mths + callable  + sinkable + insured + rated + unique_sources_12 + 
            ...
            data = full_data[rolling_sum_monthly_article_count_12 > 0], 
...
r2 <- feols(offering_yield_spread ~ city_go_vote*high_articles_12_0  + maturity_mths + callable  + sinkable + insured + rated +
              unique_sources_12 + ln_gdp + ln_pop +  ln_pers_inc  + ln_emp + city_rev_vote + state_go_vote + glm_proactive + state_ltgo_allowed |ym + purp_broad + amount_quintile,
            data = full_data[rolling_sum_monthly_article_count_12 > 0], 
            vcov = vcov_cluster(~issue_id))
summary(r2)
r3 <- feols(offering_yield ~ city_go_vote*high_articles_12_0  + city_rev_vote + maturity_mths + callable  + sinkable + insured + 
              rated + unique_sources_12 + ln_gdp + ln_pop +  ln_pers_inc  + ln_emp + 
              city_rev_vote + state_go_vote |group + ym + purp_broad + amount_quintile, 
            data = border_full_data[rolling_sum_monthly_article_count_12 > 0], 
            vcov = vcov_cluster(~issue_id))
summary(r3)
...
writeLines(modified_output, paste0(tbl_dir, '/bond_yields.tex'))
