...
issuers <- unique(data[, .(seed_issuer_id, issuer_long_name)])
...
stargazer(desc_col, summary = F,type = 'latex', no.space = T, digits = 2,
          rownames = F, table.placement = "H", out = paste0(tables_wd, '/Ravenpack Descriptives.tex'))
...
r3 <- felm(total_rp_articles_1_1 ~ city_go_vote + unique_sources_12 + ln_amount  + ln_gdp + ln_pers_inc + ln_percap_inc + ln_employment|year + purp_broad|0|state, data = issuance_lvl[go_unlim_bond_issuance == 1],psdef = FALSE)
...
stargazer(r1,r2, r3,
          ...
          out = paste0(tables_wd, '/Article Counts FINAL 250327.tex')
          )
